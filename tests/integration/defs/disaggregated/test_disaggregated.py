...
def get_test_config(test_desc, example_dir, test_root):
    ...
    config_map = {
        ...
        "llama_eagle_single_gpu":
        (2, f"{test_configs_root}/disagg_config_llama_eagle_tp1.yaml"),
    }
            ...
            for _ in range(num_iters):
                ...
                for output_file in output_files:
                    with open(output_file, 'r') as f:
                        content = f.read()
                        if "deepseek_v3_lite" in test_desc or "llama_eagle" in test_desc or output_file == "output_chat.json":
...
@pytest.mark.parametrize("llama_model_root", ['llama-3.1-8b-instruct-hf'],
                         indirect=True)
@pytest.mark.parametrize("eagle_model_roots", ['EAGLE3-LLaMA3.1-Instruct-8B'],
                         indirect=True)
def test_disaggregated_llama_eagle_single_gpu(disaggregated_test_root,
                                              disaggregated_example_root,
                                              llm_venv, llama_model_root,
                                              eagle_model_roots):
    src_dst_dict = {
        llama_model_root:
        f"{llm_venv.get_working_directory()}/Llama-3.1-8B-Instruct",
        eagle_model_roots:
        f"{llm_venv.get_working_directory()}/EAGLE3-LLaMA3.1-Instruct-8B",
    }
    for src, dst in src_dst_dict.items():
        if not os.path.islink(dst):
            os.makedirs(os.path.dirname(dst), exist_ok=True)
